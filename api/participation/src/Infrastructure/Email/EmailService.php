...
class EmailService
{
    ...
    {
        $this->smtpHost = $_ENV['SMTP_HOST'] ?? $_SERVER['SMTP_HOST'] ?? getenv('SMTP_HOST') ?: 'mail.gotribe.co';
        $this->smtpPort = (int)($_ENV['SMTP_PORT'] ?? $_SERVER['SMTP_PORT'] ?? getenv('SMTP_PORT') ?: 465);
        $this->smtpUsername = $_ENV['SMTP_USERNAME'] ?? $_SERVER['SMTP_USERNAME'] ?? getenv('SMTP_USERNAME') ?: 'tribe@gotribe.co';
        $this->smtpPassword = $_ENV['SMTP_PASSWORD'] ?? $_SERVER['SMTP_PASSWORD'] ?? getenv('SMTP_PASSWORD') ?: '7tak5K1rWyacJ42O';
        $this->fromEmail = $_ENV['SMTP_FROM_EMAIL'] ?? $_SERVER['SMTP_FROM_EMAIL'] ?? getenv('SMTP_FROM_EMAIL') ?: 'tribe@gotribe.co';
        $this->fromName = $_ENV['SMTP_FROM_NAME'] ?? $_SERVER['SMTP_FROM_NAME'] ?? getenv('SMTP_FROM_NAME') ?: 'GoTribe';
        
        // Log SMTP configuration for debugging
        error_log("EmailService - SMTP Host: {$this->smtpHost}");
        error_log("EmailService - SMTP Port: {$this->smtpPort}");
        error_log("EmailService - SMTP Username: {$this->smtpUsername}");
    }
